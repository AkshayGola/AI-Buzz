...
def main():
    ...
    MODEL_GESTURE = "C:\AI-buzz\project_main\DT_gesture_model_kailash.pkl"
    ...
    with open('model/gesture_classifier/gesture_classifier_label_kailash.csv',
    ...
    def open_camera_():
        ...
        fps = cvFpsCalc.get()
        number, mode = change_mode_kailash(key, mode)
        ...
        if score > SCORE_THRESHOLD:
            ...
            landmark_list = calc_landmark_list_new_kailash(debug_frame, hand_landmarks)     #newmodelchange

            # Conversion to relative coordinates / normalized coordinates
            # pre_processed_landmark_list = pre_process_landmark_akshay(
            #     landmark_list)

            pre_processed_landmark_list = pre_process_landmark_kailash(landmark_list)

            # Write to the dataset file
            #logging_csv(number, mode, pre_processed_landmark_list)
            logging_csv_kailash(number, mode, pre_processed_landmark_list)                        #newmodelchange

            #hand_sign_id = Dtree.predict((np.array(pre_processed_landmark_list)).reshape(1, 42))
            hand_sign_id = Dtree.predict((np.array(pre_processed_landmark_list)).reshape(1, 270))
...
def logging_csv(number, mode, landmark_list):
    ...
    if mode == 1 and (0 <= number <= 9):
        ...
        csv_path = 'model/gesture_classifier/gesture_train_kailash.csv'   #newmodelchange
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])
    return

def logging_csv_kailash(number, mode, landmark_list):                     #newmodelchange
    if mode == 0:
        pass
    if mode == 1 and (0 <= number <= 26):
        csv_path = 'model/keypoint_classifier/keypoint_kailash.csv'
        with open(csv_path, 'a', newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])
...
def calc_landmark_list_new_kailash(image, landmarks):
    image_depth, image_width, image_height = image.shape[2], image.shape[1], image.shape[0]
    ...
    for _, landmark in enumerate(landmarks):
        landmark_x = min(int(landmark[0] * image_width), image_width - 1)
        landmark_y = min(int(landmark[1] * image_height), image_height - 1)
        landmark_z = min(int(landmark[2] * image_depth), image_depth - 1)
...
def pre_process_landmark_kailash(landmark_list):
    ...
    first_point = [landmark_list[1], landmark_list[5],landmark_list[9],landmark_list[13],landmark_list[17]]
    second_point = [landmark_list[2], landmark_list[6],landmark_list[10],landmark_list[14],landmark_list[18]]
    third_point = [landmark_list[3], landmark_list[7],landmark_list[11],landmark_list[15],landmark_list[19]]
    fourth_point = [landmark_list[4], landmark_list[8],landmark_list[12],landmark_list[16],landmark_list[20]]

    thumb = [landmark_list[1], landmark_list[2], landmark_list[3], landmark_list[4]]
    index = [landmark_list[5], landmark_list[6], landmark_list[7], landmark_list[8]]
    middle = [landmark_list[9], landmark_list[10], landmark_list[11], landmark_list[12]]
    ring = [landmark_list[13], landmark_list[14], landmark_list[15], landmark_list[16]]
    pinky = [landmark_list[17], landmark_list[18], landmark_list[19], landmark_list[20]]


    #distance from base point
    for point in range(1, len(landmark_list)):
        rel_dist.append(landmark_list[point][0] - landmark_list[0][0])
        rel_dist.append(landmark_list[point][1] - landmark_list[0][1])
        rel_dist.append(landmark_list[point][2] - landmark_list[0][2])

    #distance between each point in a finger.
    fingers = [thumb, index, middle, ring, pinky]
    for finger in fingers:
        for point in range(0, len(finger)):
            for point2 in range(point + 1, len(finger)):
                rel_dist.append(finger[point][0]-finger[point2][0])
                rel_dist.append(finger[point][1]-finger[point2][1])
                rel_dist.append(finger[point][2]-finger[point2][2])

    # distance between points on the same line
    all_points = [first_point, second_point, third_point, fourth_point]
    for points in all_points:
        for point in range(0, len(points)):
            for point2 in range(point +1, len(points)):
                rel_dist.append(points[point][0]-points[point2][0])
                rel_dist.append(points[point][1]-points[point2][1])
                rel_dist.append(points[point][2]-points[point2][2])
...
def change_mode_kailash(key, mode):         #newmodelchange
    number = -1
    if key == -1:
        return number, mode
    if 48 <= key <= 57:  # 0 ~ 9
